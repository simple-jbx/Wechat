<%@ page language="java" pageEncoding="UTF-8" import="bean.YH,bean.GJZXJ,service.commonService"%>
   	...
   	<c:if test="${yh.getRYDM() == 'root'}">  
		...
		<script>
			$("#btn_add").click(function () {
				 var index = $('#table').bootstrapTable().length;
			            ...
			            index: 0,
			            row: {xymc:'-',rydm:'-',xm:'-',nj:'-',zymc:'-',yhkh:'',je:'',dj:''}
			        })
			})
		</script>
		...
		<script>
			$("#btn_clear").click(function () {
				 $.ajax({
	                    type: "post",
	                    url : '<%=session.getAttribute("basePath")%>/GJZXJController.do',
	                    data: {
	                    	 "op": "clear"
	                    },
	                    dataType: 'JSON',
		...
		<!-- 设置上传相关参数 -->
   		<script>  
		    $("#fileUpload").fileinput({  
		        language : "zh",//设置语言  
		        uploadUrl: "<%=session.getAttribute("basePath")%>/upLoadServlet.do?tableName=gjzxj",//上传地址  
		        textEncoding: 'utf-8',

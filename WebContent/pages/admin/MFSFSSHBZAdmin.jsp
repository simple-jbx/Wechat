   	...
   	<c:if test="${yh.getRYDM() == 'root'}">  
		...
		<script>
			$("#btn_add").click(function () {
				 var index = $('#table').bootstrapTable().length;
			            ...
			            index: 0,
			            row: {xm:'-',xh:'-',xymc:'-',zymc:'-',nj:'-',yhkh:'',je:''}
			        })
			})
		</script>
		...
		<script>
			$("#btn_clear").click(function () {
				 $.ajax({
	                    type: "post",
	                    url : '<%=session.getAttribute("basePath")%>/MFSFSSHBZController.do',
	                    data: {
	                    	 "op": "clear"
	                    },
	                    dataType: 'JSON',
		...
		<!-- 设置上传相关参数 -->
   		<script>  
		    $("#fileUpload").fileinput({  
		        language : "zh",//设置语言  
		        uploadUrl: "<%=session.getAttribute("basePath")%>/upLoadServlet.do?tableName=mfsfsshbz",//上传地址  
		        textEncoding: 'utf-8',
